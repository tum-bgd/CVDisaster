...
class CVDisasterDatasetTrain(Dataset):
    ...
    def __init__(self,
        ...
        self.df = pd.read_csv(f'{data_folder}/train_{small_training}.csv')
        
        self.df = self.df.rename(columns={"name1": "sat", "name2": "ground"})
...
class CVDisasterDatasetEval(Dataset):
    ...
    def __init__(self,
        ...
        if split == 'train':
            ...
            self.df = pd.read_csv(f'{data_folder}/val_{small_training}.csv')
